...
export async function startGoogleDriveOAuth(): Promise<string> {
  return invoke("start_google_drive_oauth");
}

export async function completeGoogleDriveOAuth(state: string): Promise<boolean> {
  return invoke("complete_google_drive_oauth", { state });
}

export async function backupToGoogleDrive(folderId?: string): Promise<string> {
  return invoke("backup_to_google_drive", { folderId: folderId || null });
}

export async function restoreLatestFromGoogleDrive(folderId?: string): Promise<void> {
  await invoke("restore_latest_from_google_drive", { folderId: folderId || null });
}
